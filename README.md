...
После восстановления MQTT коннектор не шлёт «текущие» значения в `data_set` до ответа платформы `prsConnector.full_configuration`: платформа сначала пишет в теги `null` с качеством 101. Накопленный буфер разрыва (исходные метки времени) можно отправить сразу после CONNECT. После разбора первого конфига сессии ядро заново читает все привязанные теги и отправляет снимок, даже если значение на источнике не изменилось.
